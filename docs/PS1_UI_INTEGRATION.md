...
Use the existing backend `docs/PS1_PERSISTENCE_TEST_GUIDE.md` first. Apply migration 0011/current head to your development database, seed the development planner identity, then start FastAPI. The UI requires the existing CORS settings to allow its local origin. The default UI backend is `http://127.0.0.1:8000`.
