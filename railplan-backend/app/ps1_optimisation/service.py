...
from .contracts import OptimiseInput, ScenarioBOptimiseInput, ScenarioCOptimiseInput, OptimiseResult
from .preprocessing import prepare
from .scenario_a import solve as solve_a
from .scenario_b import solve as solve_b
from .scenario_c import solve as solve_c
...
def _optimise(dataset, options, scenario):
    ...
    result.eclo_windows=solved.get('eclo_windows',{})
    result.cross_line_eclo_activities=solved.get('cross_line_eclo_activities',[])
    ...
    result.capacity_hotspots=_hotspots(report,dataset,mapping,scenario)
    ...
    if scenario=='B' and report['objective_score'] is not None:
        ...
        if Decimal(report['objective_score']) != expected:
            ...
            result.validation_report=ValidationReport.model_validate(report)
    if scenario=='C' and report['objective_score'] is not None:
        components=report['objective_components']
    ...
    if not (report['feasible'] and report['physical_validation_complete'] and not report['hard_violations']):
        ...
        result.failed_candidate={'submission_files':files,'physical_nights':[p.model_dump() for p in solved['placements']],
            'eclo_windows':result.eclo_windows,'cross_line_eclo_activities':result.cross_line_eclo_activities,'diagnostic_only':True}
        return result
...
def _hotspots(report,dataset,mapping,scenario):
    ...
    for row in report['capacity_hotspots']:
        ...
        enriched.append({**row,'available_supply':row['supply'],'used_possession_groups':row['used'],
            ...
            'hard_max_reached':scenario=='C' and row['excess']==1,
...
def optimise_scenario_c(dataset, options: ScenarioCOptimiseInput | None = None):
    return _optimise(dataset,options or ScenarioCOptimiseInput(),'C')
