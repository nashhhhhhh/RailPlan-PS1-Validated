...
def export_bundle(dataset, data, placements):
    ...
    for r in sorted(placements,key=lambda r:(r.activity_id,r.access_seq)):
        rows['SCHEDULE_ACCESS.csv'].append(dict(activity_id=r.activity_id,access_seq=r.access_seq,week=r.week,eclo=0,access_night=r.access_night))
    ...
    for contract,week in sorted(ends.items()):
        ...
        rows['RESULTS.csv'].append(dict(scenario='A',contract_number=contract,simulated_completion_date=complete.isoformat(),overrun_days=max(0,(complete-date.fromisoformat(p['planned_completion_date'])).days)))
    files={}
...
def export(dataset, data, placements):
    """Compatibility wrapper for the pure optimiser and offline callers."""
    bundle=export_bundle(dataset,data,placements)
